...
export class JuegosCreadosComponent implements OnInit {
  ...
  elegirJuego(id: number) {
    ...
    this.juegoSeleccionado = {
      ...
      fecha_creacion: new Date(juego.fecha_creacion)
        .toISOString()
        .split('T')[0],
      fecha_finalizacion: new Date(juego.fecha_finalizacion)
        .toISOString()
        .split('T')[0],
      estado: this._translateService.instant(
        juego.estado == 1 ? 'general.acitvo' : 'general.inacitvo'
      ),
  ...
  cerrarJuego() {
    ...
    this._profesprService.cerrarJuego(criteria).subscribe((data) => {
      if (data.msg == 'OK') {
        ...
        this.buscarJuegos();
        this.openSnackBar(
          this._translateService.instant('general-msg.ok-cerrar-juego'),
          'custom-snackbar_exitoso'
        );
      } else {
        this.openSnackBar(
          this._translateService.instant('general-msg.error-cerrar-juego'),
  ...
  verRevisores(id: number) {
    this._router.navigate(['/home/juegos-creados', id, 'revisores']);
  }
}
